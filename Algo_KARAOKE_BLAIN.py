...
class Joueur :
    ...
    def score(self,entier,chanson):
        if self.__score[chanson] < entier :
            self.__score[chanson] = entier
        return self.__score[chanson]
    def getMoyenne(self):
        moyenne = 0
        compte = 0
        for i in range (album):
    ...
    def getBest(self):
        memoire = 0
        for i in range (album):
            if self.__score[i] >memoire:
                memoire = self.__score[i]
        return memoire
    def getWorst(self):
        memoire = self.getBest()
        for i in range (album):
            if self.__score[i] <memoire and self.__score[i] !=0 :
                memoire = self.__score[i]
        return memoire
